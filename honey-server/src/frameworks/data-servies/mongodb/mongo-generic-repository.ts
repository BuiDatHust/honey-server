...
export class MongoGenericRepository<T> implements IGenericRepository<T> {
  ...
  getOneWithPopulateField(
    filter,
    populateField: { path: string; select?: string; strictPopulate?: boolean }[],
  ): Promise<any> {
    return this._repository.findOne(filter).lean().populate(populateField)
  }

  getAllByPaginate(filter, option: TPaginateOption) {
    return this._repository.find(filter).limit(option.limit).sort(option.order).lean().exec()
  }
}
